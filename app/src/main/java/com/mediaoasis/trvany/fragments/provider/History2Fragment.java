...
import com.mediaoasis.trvany.adapters.History2Adapter;
...
public class History2Fragment extends Fragment {
    ...
    History2Adapter historyAdapter;
            ...
            public void onFinish() {
                if (Main2Activity.isOrderLoaded) {
                    ...
                    for (int i = 0; i < Main2Activity.AllOrders.size(); i++) {
                        if (Main2Activity.AllOrders.get(i).getStatus().contains("canceled")
                                || Main2Activity.AllOrders.get(i).getStatus().contains("done")) {
                            InActiveOrders.add(Main2Activity.AllOrders.get(i));
                        }
                    }

                    progressBar.setVisibility(View.INVISIBLE);
                    historyAdapter = new History2Adapter(getActivity(), InActiveOrders);
                    orders_rv.setAdapter(historyAdapter);
                    if (InActiveOrders.size() == 0)
                        numOfAppointments_tv.setText(R.string.no_history_yet);
                    else
